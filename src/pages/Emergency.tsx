...
// id → data.json; styling only here, no digits.
const EMERGENCY_NUMBERS = [
 { id: 'emergency-112', labelKey: 'emergency.numbers.national', color: 'bg-red-600 hover:bg-red-700', textColor: 'text-red-600', bgLight: 'tone-red border' },
 { id: 'ambulance-108', labelKey: 'emergency.numbers.ambulance', color: 'bg-orange-600 hover:bg-orange-700', textColor: 'text-orange-600', bgLight: 'tone-orange border' },
 { id: 'police-100', labelKey: 'emergency.numbers.police', color: 'bg-blue-700 hover:bg-blue-800', textColor: 'text-blue-700', bgLight: 'tone-blue border' },
];
  ...
  <div className="flex flex-col gap-5">
   ...
   <section>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
     {EMERGENCY_NUMBERS.map(({ id, labelKey, color, textColor, bgLight }) => {
      const value = findContact(data, id)?.value ?? '';
      return (
       <div
        key={id}
   ...
   <section>
    ...
    <div className="bg-red-600 rounded-card px-3 py-2.5 mb-3">
     <p className={`text-white font-bold text-sm leading-relaxed ${isML ? 'ml-text' : ''}`}>
      {t('emergency.overdose.callFirst', tokens)}
     </p>
    </div>
